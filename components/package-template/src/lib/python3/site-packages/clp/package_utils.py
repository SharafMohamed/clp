...
def generate_container_config(clp_config: CLPConfig, clp_home: pathlib.Path):
    ...
    if rc_path.is_file:
        ...
        for line in lines:
            ...
            if key == 'schema-path':
                # only error on non-empty values not in the container
                if values and values[0].rstrip() and not values[0].startswith('/opt/clp/etc'):
                    raise ValueError(f"schema-path in clp.rc ({values}) not within /opt/clp/etc.")
