...
namespace compressor_frontend {
    ...
    template <typename NFAStateType, typename DFAStateType>
    Token Lexer<NFAStateType, DFAStateType>::scan_new (LogInputBuffer& input_buffer) {
        DFAStateType* state = m_dfa->get_root();
        if (m_asked_for_more_data) {
            ...
            if (m_match) {
                ...
                m_last_match_pos = m_match_pos;
                m_last_match_line = m_match_line;
                return Token{m_start_pos, m_match_pos, input_buffer.storage().get_active_buffer(),
                             input_buffer.storage().size(), m_match_line,
                             m_type_ids};
            }
        ...
        while (true) {
            ...
            if ((m_is_delimiter[next_char] || input_buffer.log_fully_consumed() || !m_has_delimiters) && state->is_accepting()) {
                m_match = true;
                m_type_ids = &(state->get_tags());
                m_match_pos = prev_byte_buf_pos;
                m_match_line = m_line;
            ...
            if (input_buffer.log_fully_consumed() || next == nullptr) {
                if (m_match) {
                    ...
                    m_line = m_match_line;
                    if (m_last_match_pos != m_start_pos) {
                        return Token{m_last_match_pos, m_start_pos,
                    ...
                    m_last_match_pos = m_match_pos;
                    m_last_match_line = m_match_line;
                    return Token{m_start_pos, m_match_pos,
                                 input_buffer.storage().get_active_buffer(),
                                 input_buffer.storage().size(),
                                 m_match_line, m_type_ids};
                } else if (input_buffer.log_fully_consumed() &&
                           m_start_pos == input_buffer.storage().pos()) {
                    if (m_last_match_pos != m_start_pos) {
                        m_match_pos = input_buffer.storage().pos();
                        m_type_ids = &cTokenEndTypes;
                    ...
                    return Token{input_buffer.storage().pos(), input_buffer.storage().pos(),
                                 input_buffer.storage().get_active_buffer(),
                                 input_buffer.storage().size(), m_line, &cTokenEndTypes};
                } else {
                    /// TODO: remove timestamp from m_is_fist_char so that m_is_delimiter check not needed
                    while (input_buffer.log_fully_consumed() == false && (m_is_first_char[next_char] == false | m_is_delimiter[next_char] == false)) {
                        if (input_buffer.all_data_read()) {
                            m_asked_for_more_data = true;
                            m_prev_state = state;
                            throw runtime_error("Input buffer about to overflow");
    ...
    template <typename NFAStateType, typename DFAStateType>
    Token Lexer<NFAStateType, DFAStateType>::scan_with_wildcard_new (LogInputBuffer& input_buffer,
                                                                     char wildcard) {
        DFAStateType* state = m_dfa->get_root();
        if (m_asked_for_more_data) {
            state = m_prev_state;
        ...
        while (true) {
            ...
            if (input_buffer.log_fully_consumed() || next == nullptr) {
                ...
                if (!m_match || (m_match && m_match_pos != input_buffer.storage().pos())) {
                    return Token{m_last_match_pos, input_buffer.storage().pos(),
                                 input_buffer.storage().get_active_buffer(),
                                 input_buffer.storage().size(), m_last_match_line,
                ...
                if (m_match) {
                    ...
                    return Token{m_start_pos, m_match_pos,
                                 input_buffer.storage().get_active_buffer(),
                                 input_buffer.storage().size(), m_match_line, m_type_ids};
                }
            }
            state = next;
        }
    ...
    template <typename NFAStateType, typename DFAStateType>
    void Lexer<NFAStateType, DFAStateType>::reset_new () {
        m_last_match_pos = 0;
        m_match = false;
        m_line = 0;
        m_match_pos = 0;
