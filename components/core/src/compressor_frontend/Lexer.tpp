...
namespace compressor_frontend {
    ...
    template <typename NFAStateType, typename DFAStateType>
    Token Lexer<NFAStateType, DFAStateType>::scan_new (InputBuffer& input_buffer) {
        DFAStateType* state = m_dfa->get_root();
    ...
    template <typename NFAStateType, typename DFAStateType>
    Token Lexer<NFAStateType, DFAStateType>::scan_with_wildcard_new (InputBuffer& input_buffer, char wildcard) {
        DFAStateType* state = m_dfa->get_root();
        if (m_asked_for_more_data) {
            state = m_prev_state;
            m_asked_for_more_data = false;
